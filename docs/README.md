# CouponTracker Documentation

Start with the knowledge base. It turns the large docs folder into two reading
levels: high-level orientation and detailed topic indexes.

## Knowledge Base

- [Knowledge base entry point](KNOWLEDGE_BASE.md)
- [High-level knowledge base](knowledge-base/high-level/README.md)
- [Detailed knowledge base](knowledge-base/detailed/README.md)
- [Full project knowledge diary](PROJECT_KNOWLEDGE_DIARY.md)

## Current Core Docs

- [Implementation status](IMPLEMENTATION_STATUS.md)
- [LLM integration](LLM_INTEGRATION.md)
- [AI editing checklist](ai_guardrails/AI_EDITING_CHECKLIST.md)
- [Coupon extraction rules](ai_guardrails/COUPON_EXTRACTION_RULES.md)
- [Refactor documentation](refactor/README.md)

## Current Topic Areas

- [Extraction and OCR](knowledge-base/detailed/extraction-and-ocr.md)
- [Models and local AI](knowledge-base/detailed/models-and-ai.md)
- [Android app architecture](knowledge-base/detailed/android-app.md)
- [Refactor and code rules](knowledge-base/detailed/refactor-and-rules.md)
- [Training and annotation](knowledge-base/detailed/training-and-annotation.md)
- [Store, privacy, and publishing](knowledge-base/detailed/store-and-publishing.md)
- [Git, branches, and maintenance](knowledge-base/detailed/git-and-branches.md)
- [Global AI engineering principles](knowledge-base/detailed/global-ai-engineering-principles.md)
- [Historical archive](knowledge-base/detailed/historical-archive.md)

## Historical Docs

The [archive](archive/README.md) folder contains historical development
documentation. Use it for context only. Current code and current knowledge-base
docs win when old docs conflict with current behavior.

## Getting Started

For setup and usage instructions, see the root [README](../README.md).
