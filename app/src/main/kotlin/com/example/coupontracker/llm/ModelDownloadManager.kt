...
class ModelDownloadManager(
    ...
    companion object {
        ...
        private const val QWEN25_MODEL_FILE = ModelPaths.QWEN25_MODEL_FILE
        private const val QWEN25_TOKENIZER_FILE = ModelPaths.TOKENIZER_JSON
        ...
        private const val QWEN2_MODEL_FILE = ModelPaths.QWEN2_MODEL_FILE
        ...
        private const val QWEN25_CONFIG_CREATED_AT_TOKEN = "%CREATED_AT%"
        private val QWEN25_CONFIG_TEMPLATE = """
            {
              "model": {
                "name": "Qwen2.5-1.5B-Instruct",
                "architecture": "llama",
                "quantization": "Q4_K_M",
                "description": "Autogenerated runtime configuration for mobile coupon extraction.",
                "source": "bartowski/Qwen2.5-1.5B-Instruct-GGUF",
                "context_window": 8192,
                "rope_scaling": {
                  "type": "llama",
                  "base": 10000,
              ...
              "prompt_template": {
                ...
                "ensure_final_eos": true,
                "default_system_message": "You are a helpful assistant that outputs concise JSON without extra commentary."
              },
              "sampling": {
                "temperature": 0.1,
                "top_p": 0.85,
              ...
              "grammar": {
                "file": "coupon_schema.gbnf",
                "enabled": true,
                "fallback_behavior": "reject"
              },
              "postprocessing": {
                "strip_json_wrapper": true,
                "coerce_valid_json": true,
                "repair_common_errors": true
              },
              "telemetry": {
                "schema_version": 1,
                "build": "auto-generated-mobile-config",
                "created_at": "$QWEN25_CONFIG_CREATED_AT_TOKEN",
                "notes": "This file is safe to regenerate. Update SEGMENTED_PROMPTS or sampling to adjust behaviour."
              }
            }
        """.trimIndent()
    ...
    suspend fun downloadQwen25Model(
        ...
        try {
            ...
            // ⭐ NEW: Ensure tokenizer is present (download from repo if needed)
            val tokenizerFile = File(modelDir, QWEN25_TOKENIZER_FILE)
            if (!tokenizerFile.exists() || tokenizerFile.length() < ModelPaths.getMinFileSize(modelId, QWEN25_TOKENIZER_FILE)) {
                ...
                tokenizerResult.getOrElse { error ->
                    tokenizerFile.delete()
                    return@withContext DownloadResult.Error("Tokenizer download failed: ${resolveErrorMessage(error)}")
                }
                if (tokenizerFile.length() < ModelPaths.getMinFileSize(modelId, QWEN25_TOKENIZER_FILE)) {
                    tokenizerFile.delete()
                    return@withContext DownloadResult.Error("Tokenizer file too small (${tokenizerFile.length()} bytes)")
                }
                Log.d(TAG, "Tokenizer downloaded: ${tokenizerFile.length()} bytes")
            } else {
                Log.d(TAG, "Tokenizer already present (${tokenizerFile.length()} bytes)")
            }

            // ⭐ NEW: Generate mlc-chat-config.json required by runtime bridge
            val configFile = File(modelDir, QWEN25_CONFIG_FILE)
            val minConfigSize = ModelPaths.getMinFileSize(modelId, QWEN25_CONFIG_FILE)
            if (!configFile.exists() || configFile.length() < minConfigSize) {
                progressCallback(DownloadProgress(99, "Writing runtime config..."))
                writeQwen25ConfigFile(configFile)
                if (configFile.length() < minConfigSize) {
                    configFile.delete()
                    return@withContext DownloadResult.Error("Config file generation failed (${configFile.length()} bytes)")
                }
            } else {
                Log.d(TAG, "Config already present (${configFile.length()} bytes)")
            }
            
            // Create .verified marker
            val verifiedMarker = File(modelDir, ".verified")
            verifiedMarker.writeText(
                buildString {
                    appendLine("Qwen2.5 Model verified: $QWEN25_VERSION")
                    appendLine("Timestamp: ${System.currentTimeMillis()}")
                    appendLine("Files: $QWEN25_MODEL_FILE, $QWEN25_CONFIG_FILE, $QWEN25_TOKENIZER_FILE, coupon_schema.gbnf")
                }
            )
    ...
    private fun writeQwen25ConfigFile(configFile: File) {
        configFile.parentFile?.mkdirs()
        val payload = QWEN25_CONFIG_TEMPLATE.replace(
            QWEN25_CONFIG_CREATED_AT_TOKEN,
            System.currentTimeMillis().toString()
        )
        configFile.writeText(payload)
        Log.d(
            TAG,
            "✅ Generated mlc-chat-config.json (${payload.length} bytes) at ${configFile.absolutePath}"
        )
    }
