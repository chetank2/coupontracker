...
class IndiaCouponModelManager(private val context: Context) {
    ...
    companion object {
        ...
        private val DEFAULT_CLASSES = listOf(
            "store_name",
            "coupon_code",
            "expiry_date",
            "description",
    ...
    /**
     * Initialize the model if it has not yet been prepared.
     */
    fun initialize() {
        if (interpreter != null) {
            return
        }
        initializeFromBundle(modelManager.active())
    }
    ...
    private fun initializeFromBundle(bundle: ModelManager.ModelBundle) {
        ...
        try {
            ...
            if (config != null) {
                inputSize = config.optInt("input_size", DEFAULT_INPUT_SIZE)
                classes = config.optJSONArray("classes")?.let { array ->
                    (0 until array.length()).map { array.getString(it) }
                } ?: DEFAULT_CLASSES
                confidenceThreshold = config.optDouble("confidence_threshold", confidenceThreshold.toDouble()).toFloat()
                iouThreshold = config.optDouble("iou_threshold", iouThreshold.toDouble()).toFloat()
            } else {
                classes = DEFAULT_CLASSES
            }

            val modelFile = copyAssetToCache(bundle, ModelFile.MODEL)
            if (modelFile == null) {
                Log.e(TAG, "Failed to prepare model file for bundle ${bundle.directory}")
                return
            }
            ...
            Log.i(TAG, "Model initialized successfully with ${classes.size} classes from bundle ${bundle.directory}")
        } catch (e: Exception) {
            Log.e(TAG, "Error initializing model from bundle ${bundle.directory}", e)
            releaseInterpreter()
        }
    }
    ...
    private fun copyAssetToCache(bundle: ModelManager.ModelBundle, file: ModelFile): File? {
        return try {
            val relativePath = modelManager.getFilePath(bundle, file)
            val fileName = relativePath.substringAfterLast("/")
            val cacheFile = File(context.cacheDir, "${bundle.directory}_$fileName")
            if (!cacheFile.exists()) {
                modelManager.openFile(bundle, file).use { input ->
            ...
            Log.e(TAG, "Error copying asset to cache: ${file.key}", e)
            null
        }
    }
