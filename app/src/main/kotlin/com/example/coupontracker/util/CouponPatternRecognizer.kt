...
class CouponPatternRecognizer(private val context: Context) {
    ...
    private var patterns: Map<String, List<Rect>> = emptyMap()

    // Initialization flag
    private val isInitialized = AtomicBoolean(false)

    private val bundleListener = ModelManager.ModelBundleListener {
        patterns = emptyMap()
     ...
     */
    private fun loadPatterns(bundle: ModelManager.ModelBundle): Map<String, List<Rect>> {
    ...
    suspend fun recognizeElements(bitmap: Bitmap): Map<String, String> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.d(
                TAG,
                "Pattern recognition summary – matched ${result.keys} with confidences $confidenceScores"
            )
    ...
    /**
     * Process image with pattern-based text recognition
     */
    private suspend fun processWithSpecializedConfig(bitmap: Bitmap, type: String): String {
        return try {
            // Extract text from bitmap using Android's built-in text recognition capabilities
            ...
            Log.e(TAG, "Error processing image for $type", e)
            ""
        }
    }

    /**
     * Extract text from bitmap using pattern recognition
     */
    private fun extractTextFromBitmap(bitmap: Bitmap): String {
        // This is a simplified implementation that uses the pattern recognizer's capabilities
        // In a real implementation, this would use a trained model for text recognition

        // Convert bitmap to grayscale for better text extraction
        ...
        // Create a new bitmap with the grayscale pixels
        val grayscaleBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        grayscaleBitmap.setPixels(pixels, 0, width, 0, 0, width, height)

        // Use the trained model's pattern recognition to extract text
        // For now, we'll use a simple approach based on the bitmap's characteristics

        // Count dark pixels to estimate text density
        var darkPixelCount = 0
        for (pixel in pixels) {
            val gray = android.graphics.Color.red(pixel) // All RGB values are the same in grayscale
            if (gray < 128) { // Threshold for dark pixels
                darkPixelCount++
            }
        }

        // Calculate text density
        val textDensity = darkPixelCount.toFloat() / pixels.size

        // Log the text density for debugging
        Log.d(TAG, "Text density: $textDensity")

        // Return a placeholder text based on the bitmap characteristics
        // In a real implementation, this would use the trained model
        return "Text from bitmap ${width}x${height}"
    }
    ...
    fun convertToCouponInfo(elements: Map<String, String>): CouponInfo {
        ...
        if (storeName.isNotBlank()) {
            fieldConfidences["storeName"] = calculateConfidenceScore(storeName, "store").coerceIn(0f, 1f)
        }
        if (!redeemCode.isNullOrBlank()) {
            fieldConfidences["code"] = calculateConfidenceScore(redeemCode, "code").coerceIn(0f, 1f)
        }
        if (!amountStr.isNullOrBlank()) {
            fieldConfidences["amount"] = calculateConfidenceScore(amountStr, "amount").coerceIn(0f, 1f)
        }
        if (!expiryDateStr.isNullOrBlank()) {
            fieldConfidences["expiryDate"] = calculateConfidenceScore(expiryDateStr, "expiry").coerceIn(0f, 1f)
        }
        if (description.isNotBlank()) {
            fieldConfidences["description"] = calculateConfidenceScore(description, "description").coerceIn(0f, 1f)
        }

        // Determine discount type
        val discountType = when {
            amountStr != null && amountStr.contains("%") -> "PERCENTAGE"
            cashbackAmount != null && cashbackAmount > 0 -> "AMOUNT"
            else -> null
        }

        val benefitType = when {
            discountType.equals("PERCENTAGE", ignoreCase = true) -> "percentage"
            cashbackAmount != null && cashbackAmount > 0 -> "cashback"
            else -> discountType?.lowercase(Locale.getDefault())
        }

        val benefitValue = when (benefitType) {
            "percentage" -> amountStr?.replace("%", "")?.toDoubleOrNull()
            "cashback" -> cashbackAmount
            else -> null
        }

        val currency = when (benefitType) {
            "cashback" -> "INR"
            else -> null
        }
        ...
        val activeVersion = modelManager.active().version.raw
        Log.d(TAG, "Converting elements to CouponInfo using model v$activeVersion")

        val normalizedConfidence = when {
            !redeemCode.isNullOrBlank() && benefitValue != null -> 0.85f
            !redeemCode.isNullOrBlank() || benefitValue != null -> 0.7f
            else -> 0.5f
        }
