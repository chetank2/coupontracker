    ...
    suspend fun processCouponFromBitmap(bitmap: Bitmap): Coupon {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Barcode scanning has been removed to simplify the OCR implementation

                // Process with OCR
                val couponInfo = imageProcessor.processImage(bitmap)

                // Convert CouponInfo to Coupon
                val normalizedBenefitType = couponInfo.benefitType ?: when {
                    couponInfo.discountType != null -> couponInfo.discountType
                    (couponInfo.cashbackAmount ?: 0.0) > 0 -> "cashback"
                    else -> null
                }
                val normalizedBenefitValue = couponInfo.benefitValue ?: couponInfo.cashbackAmount
                val normalizedCurrency = couponInfo.currency ?: normalizedBenefitValue?.let { "INR" }
                val normalizedExpiryIso = couponInfo.expiryIso ?: couponInfo.expiryDate.toUtcIsoString()
                val normalizedConfidence = if (couponInfo.confidence > 0f) couponInfo.confidence else 0.6f
