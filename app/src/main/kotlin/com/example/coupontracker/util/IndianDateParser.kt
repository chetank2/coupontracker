...
object IndianDateParser {
    ...
    private fun cleanDateString(rawDate: String): String {
        ...
        // Reorder month-year-day artifacts generated by LLM (e.g., "May-2025 31" -> "31 May 2025")
        val monthYearDayPattern = Regex("""\\b([A-Za-z]{3,9})[-/, ]*(20\\d{2})[-/, ]*(\\d{1,2})\\b""")
        cleaned = monthYearDayPattern.replace(cleaned) { match ->
        ...
        // Handle Month/Day/Year with explicit slashes that may not trigger word boundaries (e.g., "May/31/2025")
        cleaned = cleaned.replace(Regex("""([A-Za-z]{3,9})/(\\d{1,2})/(20\\d{2})""")) { match ->
            val month = normalizeMonthToken(match.groupValues[1])
            val day = match.groupValues[2]
            val year = match.groupValues[3]
            "$day $month $year"
    ...
    private fun normalizeMonthToken(monthToken: String): String {
        val lower = monthToken.lowercase(Locale.ENGLISH)
        return MONTH_REPLACEMENTS[lower] ?: monthToken.replaceFirstChar { ch ->
