...
data class CouponInfo(
    ...
    fun isValid(): Boolean {
        ...
        val hasCode = !redeemCode.isNullOrBlank() || !benefitType.isNullOrBlank()
        val hasValue = (benefitValue ?: cashbackAmount ?: 0.0) > 0.0
    ...
    override fun toString(): String {
        return "CouponInfo(storeName='$storeName', description='$description', " +
               "expiryDate=$expiryDate, expiryIso=$expiryIso, cashbackAmount=$cashbackAmount, " +
               "benefitValue=$benefitValue, benefitType=$benefitType, currency=$currency, " +
               ...
               "platformType=$platformType, usageLimit=$usageLimit, app=$app, confidence=$confidence, " +
...
class TextExtractor {
    ...
    fun extractCouponInfoSync(text: String): CouponInfo {
        ...
        if (!storeName.isNullOrBlank()) fieldConfidences["storeName"] = 0.7f
        if (!description.isNullOrBlank()) fieldConfidences["description"] = 0.5f
        cashbackAmount?.let { fieldConfidences["amount"] = 0.6f }
        if (!redeemCode.isNullOrBlank()) fieldConfidences["code"] = 0.7f
        expiryDate?.let { fieldConfidences["expiryDate"] = 0.6f }
        ...
        val result = CouponInfo(
            ...
            benefitValue = cashbackAmount,
            currency = cashbackAmount?.takeIf { it > 0 }?.let { "INR" },
            expiryIso = expiryDate.toUtcIsoString(),
            app = platformType,
            confidence = when {
                redeemCode.isNullOrBlank() && cashbackAmount == null -> 0.3f
                redeemCode.isNullOrBlank() || cashbackAmount == null -> 0.6f
                else -> 0.8f
            },
            fieldConfidences = fieldConfidences
        )
