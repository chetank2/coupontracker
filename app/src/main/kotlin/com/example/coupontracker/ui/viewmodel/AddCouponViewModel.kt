...
class AddCouponViewModel @Inject constructor(
    ...
    fun saveCoupon(
        ...
        viewModelScope.launch {
            ...
            try {
                val normalizedExpiry = currentExpiryDate.toUtcIsoString()
                val normalizedConfidence = 0.8f
                val coupon = Coupon(
                    storeName = storeName,
                    ...
                    code = redeemCode?.uppercase(Locale.getDefault()),
                    benefitType = when {
                        cashbackAmount > 0 -> "cashback"
                        else -> null
                    },
                    benefitValue = cashbackAmount.takeIf { it > 0 },
