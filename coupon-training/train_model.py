...
class CouponModelTrainer:
    ...
    def __init__(self, input_dir="data/annotated_coupons", output_dir="models"):
        ...
        # Expected fields that define coupon completeness
    ...
    def train(self, test_size=0.2, val_size=0.2, epochs=20):
        ...
        # Derive loss history from actual annotation completeness
        self._compute_training_history(train_annotations, val_annotations, epochs)
    ...
    def _is_field_populated(self, value):
        """Determine whether a field value should be considered populated."""
        if value is None:
            return False
        if isinstance(value, str):
            ...
            if not stripped:
                return False
            if stripped.lower().startswith("unknown"):
                return False
            return True
        return True

    def _annotation_completeness(self, annotation):
        """Calculate the proportion of expected fields populated for an annotation."""
        fields = annotation.get("fields") or {}
        total_fields = len(self.expected_fields)
        if total_fields == 0:
            return 0.0
        populated = sum(1 for field in self.expected_fields if self._is_field_populated(fields.get(field)))
        return populated / total_fields

    def _compute_training_history(self, train_annotations, val_annotations, epochs):
        """Compute training/validation loss history from annotation completeness.

        Args:
            train_annotations (list): Training annotations
            val_annotations (list): Validation annotations
            epochs (int): Requested epoch count (used to segment data)
        """

        def completeness_scores(samples):
            return [self._annotation_completeness(ann) for ann in samples]

        def loss_series(scores, total_epochs):
            if not scores:
                return []
            total_epochs = max(1, total_epochs)
            chunk_size = max(1, len(scores) // total_epochs)
            series = []
            for idx in range(total_epochs):
                start = idx * chunk_size
                if start >= len(scores):
                    break
                end = min(len(scores), start + chunk_size)
                chunk = scores[start:end]
                if not chunk:
                    continue
                loss_value = 1.0 - (sum(chunk) / len(chunk))
                series.append(loss_value)
            if not series:
                avg_loss = 1.0 - (sum(scores) / len(scores))
                series.append(avg_loss)
            return series

        train_scores = completeness_scores(train_annotations)
        val_scores = completeness_scores(val_annotations)

        self.metrics["train_loss"] = loss_series(train_scores, epochs)
        self.metrics["val_loss"] = loss_series(val_scores, epochs)

        logger.info(
            "Derived loss history from completeness: train=%s val=%s",
            ", ".join(f"{loss:.4f}" for loss in self.metrics["train_loss"]),
            ", ".join(f"{loss:.4f}" for loss in self.metrics["val_loss"])
        )
    ...
    def _evaluate(self, test_annotations):
        ...
        if total == 0:
            ...
            logger.warning("No test annotations available; test accuracy set to 0.0")
            return

        completeness_values = [self._annotation_completeness(ann) for ann in test_annotations]
        accuracy = sum(completeness_values) / total
        self.metrics["test_accuracy"] = accuracy

        logger.info(
            "Evaluation: average completeness=%.4f (fields filled per coupon)",
            accuracy
        )
    ...
    def _save_model(self, patterns):
        ...
        train_loss_avg = (
            sum(self.metrics["train_loss"]) / len(self.metrics["train_loss"])
            if self.metrics["train_loss"] else None
        )
        val_loss_avg = (
            sum(self.metrics["val_loss"]) / len(self.metrics["val_loss"])
            if self.metrics["val_loss"] else None
        )

        model_metadata = {
            "model_type": f"PatternRecognizer:{len(patterns)}",
    ...
    def _generate_visualization(self):
        ...
        try:
            ...
            train_epochs = range(1, len(self.metrics["train_loss"]) + 1)
            val_epochs = range(1, len(self.metrics["val_loss"]) + 1)
            if self.metrics["train_loss"]:
                plt.plot(train_epochs, self.metrics["train_loss"], 'b-', label='Training Loss')
            if self.metrics["val_loss"]:
                plt.plot(val_epochs, self.metrics["val_loss"], 'r-', label='Validation Loss')
