...
JAR="${PROJECT_ROOT}/extraction-tool/build/libs/extraction-tool.jar"
if [ ! -f "${JAR}" ]; then
  ./gradlew :extraction-tool:extractionToolJar
fi
...
PYTHONPATH="${PROJECT_ROOT}/scripts" python -m extraction_eval --jar "${JAR}" "$@"
